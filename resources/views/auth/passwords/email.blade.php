  ...
  <div class="container">
    ...
    @if (session('status'))
        <div class="alert alert-success" role="alert">
            ¡Le hemos enviado por correo electrónico el enlace para restablecer su contraseña!, revisar su carpeta de SPAM
        </div>
    @endif
    <form method="POST" action="{{ route('password.email') }}" name="form1" class="box" onsubmit="return checkStuff()">
      ...
      <input placeholder="{{ __('Email Address') }}" style="margin-top: 100px;" id="email" type="email" class="@error('email') is-invalid @enderror" name="email" value="{{ old('email') }}" required autocomplete="email" autofocus>
        ...
        @enderror
        <input type="submit" value="{{ __('Send Password Reset Link') }}" class="btn1">
      </form>
